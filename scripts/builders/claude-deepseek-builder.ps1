...
$config = Get-BuilderConfig -Path $configFile -BuilderId 'claude-builder'
$claude = if ($config.ContainsKey('command_path')) { $config.command_path } else { Join-Path $workspace '.npm-global\claude.cmd' }
...
if ($keys.Count -le $keyIndex) {
  throw "Missing API key index $keyIndex for claude-builder in $apiKeyFile"
}

$env:ANTHROPIC_BASE_URL = $config.base_url
$env:ANTHROPIC_AUTH_TOKEN = $keys[$keyIndex]
foreach ($name in $config.env.Keys) {
  [Environment]::SetEnvironmentVariable($name, [string]$config.env[$name], 'Process')
}
...
& $claude --print --model $config.model --add-dir $workspace --dangerously-skip-permissions $prompt
