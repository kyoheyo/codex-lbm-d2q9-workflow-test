...
$config = Get-BuilderConfig -Path $configFile -BuilderId 'bailian-opencode-builder'
$keys = Get-ApiKeys -Path $apiKeyFile
$keyIndex = [int]$config.key_index
if ($keys.Count -le $keyIndex) {
  throw "Missing API key index $keyIndex for bailian-opencode-builder in $apiKeyFile"
}
...
& opencode run --model $config.model --format json --dir $workspace --dangerously-skip-permissions $prompt
